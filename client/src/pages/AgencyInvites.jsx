  ...
  return (
    <div className="fade-in">
      <div style={{ marginBottom: 'var(--space-xl)' }}>
        <h2 style={{ marginBottom: 'var(--space-xs)' }}>Agency Invites 🏢</h2>
        <p className="text-muted">Project invites from recruiters for your agency.</p>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="empty-state">
          ...
          <p className="text-muted text-sm">Complete your agency profile to attract recruiters.</p>
        </div>
      ) : (
        filtered.map(invite => {
          ...
          return (
            <div className="card" key={invite._id} style={{ marginBottom: 'var(--space-md)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 'var(--space-sm)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-sm)' }}>
                  ...
                  {invite.status === 'pending' && (
                    <>
                      <button className="btn btn-sm" style={{ background: '#10b981', color: '#fff', border: 'none' }} onClick={() => handleRespond(invite._id, 'accepted')}>
                        <HiOutlineCheck size={14} /> Accept
                      </button>
                      <button className="btn btn-sm" style={{ background: '#ef4444', color: '#fff', border: 'none' }} onClick={() => handleRespond(invite._id, 'declined')}>
                        <HiOutlineX size={14} /> Decline
                      </button>
                    </>
                  )}
                  {invite.assignedPM && <span className="text-muted text-sm">PM: {invite.assignedPM?.name}</span>}
                  <button onClick={() => setExpandedId(isExpanded ? null : invite._id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}>
              ...
              {isExpanded && (
                <div style={{ marginTop: 'var(--space-md)' }}>
                  ...
                  {invite.status !== 'declined' && (
                    <MessageThread inviteId={invite._id} currentUserId={user?._id} />
                  )}
                </div>
              )}
            </div>
